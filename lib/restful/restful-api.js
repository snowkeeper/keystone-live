...
var debug = DEBUG('keystone-live:apiRoutes');
...
function selectFields(user, main) {
	...
	if(_.isString(user.select)) {
		select = user.select;			
	} else if(_.isString(user.exclude)) {
...
exports.list = function(list) {
	return function(req, res) {
		...
		var test = false;
		try {test=JSON.parse(find)}catch(e){debug('## list ## parse error',e)}
		if(test) find = test;
		debug('## list ## find: ', typeof options.find, find);
		if(!_.isArray(find)) {
			if(!_.isObject(find)) {
				find = [{}];
			} else {
		...
		var q = list.model.find.apply(list.model, find);
...
exports.remove = function(list) {
	...
	return function(req, res) {
		list.model.findById(req.params.id).exec(function (err, item) {
			if (err) return res.apiError('database error', err, null, 200);
			if (!item) return res.apiError('404', 'The item you requested (' + req.params.id + ') was not found.', null, 200);
